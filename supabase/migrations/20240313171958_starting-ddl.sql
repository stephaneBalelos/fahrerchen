...
alter table public.courses_subscriptions enable row level security;
...
create policy "Everyone can see course_subscriptions" on public.courses_subscriptions for select to authenticated using (public.authorize('course_subscriptions.read', organisation_id));
...
create policy "Owner, Manager can insert course_subscriptions" on public.courses_subscriptions for insert to authenticated with check (public.authorize('course_subscriptions.create', organisation_id));
...
create policy "Owner & Manager can update course_subscriptions" on public.courses_subscriptions for update to authenticated using (public.authorize('course_subscriptions.update', organisation_id)) with check (public.authorize('course_subscriptions.update', organisation_id));
...
create policy "Owner & Manager can delete course_subscriptions" on public.courses_subscriptions for delete to authenticated using (public.authorize('course_subscriptions.delete', organisation_id));
...
INSERT INTO course_types (type, description) VALUES
