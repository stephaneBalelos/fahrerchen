...
create policy "owner_manager_teacher_can_update_course_activity_schedule_requests" on public.course_activity_schedule_requests for update to authenticated using (public.authorize('course_activity_schedule_requests.update', organization_id)) with check ((public.authorize('course_activity_schedule_requests.update', organization_id) or requested_by = (select auth.uid())) and (status = 'pending'));
