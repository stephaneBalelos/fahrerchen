...
const { data: history } = await useAsyncData(
  `bills_${props.billId}_history`,
  async () => {
    const { data, error } = await client
      .from("course_subscription_bills")
      .select(
        "id, created_at, history:course_subscription_bill_history(*, actor:users(firstname, lastname))"
      )
      .eq("id", props.billId)
      .gt("history.inserted_at", props.billCreatedAt)
      .order('inserted_at', { ascending: false, referencedTable: 'course_subscription_bill_history' })
      .single();
