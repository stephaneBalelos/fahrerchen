  ...
  <UDashboardSlideover :ui="{ width: 'w-screen max-w-xl' }">
    ...
    >
      ...
      >
          ...
          <USelectMenu
            ...
            :options="
              ACTIVITY_TYPES.map((type) => ({
                label: g('courses.activities.types.' + type),
                id: type,
              }))
            "
            value-attribute="id"
            :leading-icon="
              selected_activity_type
                ? ACTIVITY_ICONS[selected_activity_type]
                : undefined
            "
          >
            <template #label>
              ...
              <div v-else>
                <span class="truncate">{{
                  t("form.activity_type_placeholder")
                }}</span>
              </div>
            </template>
        ...
        <UFormGroup
          v-if="props.organizationId && props.courseActivityId"
          :label="t('form.classes_settings')"
          :description="t('form.classes_settings_desc')"
          :ui="{ container: '' }"
        >
          <div class="py-6">
            <CoursesSettingsCourseActivityCombinationSettings
              :organization-id="props.organizationId"
              :activity-id="props.courseActivityId"
            />
          </div>
        </UFormGroup>
    ...
    <template #footer>
      <div class="flex items-center gap-4">
        <UButton @click="form?.submit()">{{ t('form.save') }}</UButton>
...
type Emits = {
  (
    event: "activity-saved" | "activity-deleted",
    payload?: AppCourseActivity
  ): void;
};
...
onMounted(async () => {
  if (props.courseActivityId) {
    ...
    try {
      const data = await courseActivitiesStore.getCourseActivity(
        props.courseActivityId
      );
...
async function updateCourseActivity(params: CourseActivityEdit) {
  ...
  try {
    await courseActivitiesStore.updateCourseActivity(
      props.courseActivityId,
      params
    );
    $emit("activity-saved");
...
{
  "de": {
    "new_course_activity": "Neue Kurs Aktivität",
    "form_section_title": "Kurs Aktivität bearbeiten",
    ...
    "form": {
      ...
      "classes_settings": "Ausbildungsklassen",
      "classes_settings_desc": "Wählen Sie die Ausbildungsklassen aus, die diese Aktivität enthalten sollen. Sie können auch spezifische Preise und erforderliche Teilnahmen für jede Klasse festlegen.",
      "close": "Schließen",
      "save": "Speichern"
    }
  },
  "en": {
    "new_course_activity": "New Course Activity",
    "form_section_title": "EditCourse Activity",
    ...
    "form": {
      ...
      "classes_settings": "Training Classes",
      "classes_settings_desc": "Select the training classes that should include this activity. You can also set specific prices and required attendances for each class.",
      "close": "Close",
      "save": "Save"
    }
  }
